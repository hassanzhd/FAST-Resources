function jacobi()
    ...
    if flag==0
        ...
        for i=1:iterations
            computedValues=zeros(1,n);
            for j=1:n
                tempArray=initialValues(1:n~=j);
                arguments=num2cell(tempArray);
                switch j
                    case 1
                      computedValues(j)=fx1(arguments{:});
                      disp(computedValues(j));
                    case 2
                      computedValues(j)=fx2(arguments{:});
                      disp(computedValues(j));
                    case 3
                      computedValues(j)=fx3(arguments{:});
                      disp(computedValues(j));
                    case 4
                      computedValues(j)=fx4(arguments{:});
                      disp(computedValues(j));
                    case 5
                      computedValues(j)=fx5(arguments{:});
                      disp(computedValues(j));             
                    case 6
                      computedValues(j)=fx6(arguments{:});
                      disp(computedValues(j));
                end
            end
            initialValues=computedValues(1:n); % computed values are copied to original array
        end
